...
export async function emitComponents(highlight: Highlight): Promise<{
  ...
  // One generated TS module per component: its own source + Shiki markup, baked
  // into the prerendered HTML by the detail page.
  //
  // One module per slug rather than one `Record<slug, …>` for all of them.
  // Highlighted markup is enormous — the combined map was 9.6 MB of source, and
  // because a `Record` literal is a single binding, importing it to read one
  // entry pulled all 152 into the route chunk. That, not the examples, was the
  // bulk of the 9.29 MB chunk measured in task #171. Split per slug, the page
  // loads only the component it is showing.
  const sourcesDir = join(SRC_DIR, "registry/sources")
  await rm(sourcesDir, { recursive: true, force: true })
  await mkdir(sourcesDir, { recursive: true })
  for (const s of sources) {
    const mod = [
      "// AUTO-GENERATED by scripts/generate-api.ts. Do not edit.",
      `export const source = ${JSON.stringify(s.source)}`,
      `export const highlighted = ${JSON.stringify(s.highlighted)}`,
      "",
    ].join("\n")
    await writeFile(join(sourcesDir, `${s.slug}.generated.ts`), mod)
  }
