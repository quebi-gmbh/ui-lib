...
async function main() {
  ...
  // llms.txt — discovery entry point for agents
  const llms = [
    "# ui-lib",
    "",
    "> quebi React component library. Copy-paste source, no install required.",
    "",
    "## Discovery",
    `- [Component index (JSON)](${BASE_URL}/api/index.json): all components, metadata, dependencies, source URLs`,
    `- [shadcn registry](${BASE_URL}/api/registry.json): use \`npx shadcn add ${BASE_URL}/r/<name>.json\``,
    "",
    "## How to use a component",
    `Fetch its source from ${BASE_URL}/api/components/<name>.tsx and add it to your project.`,
    "Resolve `registryDependencies` (other ui-lib components and shared libs, e.g. lib/utils) recursively; install `dependencies` (npm packages). Or run `npx shadcn add <url>` and the CLI resolves them for you.",
    "",
    "## Components",
    ...metaRegistry.map(
