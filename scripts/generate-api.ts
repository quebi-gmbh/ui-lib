...
async function main() {
  ...
  const urls = ["/", "/components", ...metaRegistry.map((m) => `/components/${m.slug}`)]
  const sitemap = [
  ...
  console.log(
    `Generated API for ${catalog.length} component(s): api/index.json, api/components/*, r/*, registry.json, llms.txt, sitemap.xml, robots.txt`,
  )
}
