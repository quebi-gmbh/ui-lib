...
const SheetContent = ({
  ...
  return (
    <ModalOverlay
      ...
      className="entering:fade-in exiting:fade-out fixed start-0 top-0 z-50 size-full entering:animate-in exiting:animate-out overflow-hidden bg-ink-900/40 entering:duration-500 exiting:duration-300"
      {...props}
    >
      <Modal
        data-float={isFloat}
        className={cx(
          // Cellestial DS — surface with shadow-4 + ink-100 border, matching Modal.
          "fixed z-50 grid gap-4 bg-surface text-ink-800 border border-ink-100 shadow-4",
          "transform-gpu transition ease-in-out will-change-transform [--visual-viewport-vertical-padding:16px]",
          "data-[float=true]:rounded-lg",
