      ...
      className={composeRenderProps(className, (className, { hasSubmenu, ...renderProps }) =>
        dropdownItemStyles({
          ...
          className: hasSubmenu
            ? twMerge(
                // Open-submenu state — use brand tint for the default intent
                // so it matches the hovered / focused state from dropdownItemStyles.
                intent === "danger" && "open:bg-danger-100 open:text-danger-600",
                intent === "warning" && "open:bg-warning-100 open:text-warning-600",
                intent === undefined &&
                  "open:bg-brand-50 open:text-brand-700 open:*:data-[slot=icon]:text-brand-700 dark:open:bg-primary-subtle dark:open:text-primary-subtle-fg dark:open:*:data-[slot=icon]:text-primary-subtle-fg",
                className,
              )
            : className,
        }),
