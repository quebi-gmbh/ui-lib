...
// `aria-hidden` is not in here: these icons are decorative — the status ones
// restate the toast's intent, and CloseIcon sits inside a button that is
// already labelled "Dismiss notification" — and lint/a11y/noSvgWithoutTitle
// cannot see an attribute arriving through a spread. So each <svg> states it
// literally, before the spreads, where a caller that needs a labelled icon can
// still override it.
...
export function ToastProvider({
  ...
  const viewport =
    typeof document !== "undefined"
      ? createPortal(
          // A named <section> is the region landmark, so the role is the
          // element rather than an attribute on a div.
          <section
