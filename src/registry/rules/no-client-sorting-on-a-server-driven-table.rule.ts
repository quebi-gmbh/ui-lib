 ...
 *
 * Note the name. It was chosen over "server-paginated" when the component's
 * props had no pagination at all; they have since gained `page`, `pageSize`,
 * `total`, `hasMore` and cursors, which makes the original title the accurate
 * one twice over: the rows are now explicitly a page of an answer, so sorting
 * them in the browser reorders a slice rather than a dataset. The name still
 * says "server-driven" rather than "server-paginated" because the rule holds
 * for an unpaginated ServerTable too — the filters and the search are queries
 * either way.
 *
 * The component caught up with the rule rather than the other way round. This
 * page has said "server-driven" throughout since it was written, while the
 * thing it described was called `AsyncTable` — a name on a different axis
 * ("how the rows arrive") that was also false, since the client-side table has
 * `isLoading`, `isRefreshing`, `error` and `onRetry` and works fine over a
 * fetch. The element this matches is `ServerTable` now, and the two vocabularies
 * agree.
 */
export const noClientSortingOnAServerDrivenTableRule: RuleMeta = {
  ...
  summary:
    "ServerTable is controlled: it reports the whole new query through onQueryChange and draws its header from the query it was given. Re-query and pass the rows back. If the whole dataset is already in the browser, that is a DataTable, not a ServerTable.",
  severity: "warn",
  ...
  rationale: [
    "The rows are an answer, not the table. ServerTable's filter popovers load their values through `loadFilterValues`, which is a call to the server — so by the time you are sorting in the client, half the table is already the server's opinion and half is yours. And the query behind those rows has a `LIMIT`: `pageSize` is in `DataTableQuery`, so what you were handed is page 3 of something. Sorting it puts the largest of those twenty rows on top and calls it the largest, which it is not.",
...
/>`,
  ...
  enforcement: {
    kind: "lint",
    // The conjunction is the rule: a sort, and a <ServerTable> in the same
  ...
  $call <: within bubble or {
    ...
    message:
      "ServerTable is controlled: it reports the whole new query through onQueryChange and draws its header from the query it was given, so sorting the rows here reorders one page of the answer to the last query instead of asking for a new one. Put the query in the URL from onQueryChange (queryToSearchParams), sort in the loader's ORDER BY, and pass rows and query back down. If the whole dataset is already in the browser, use DataTable from @/components/data-table — same columns, and the row model does the sorting on purpose. See https://ui-lib.quebi.de/rules/no-client-sorting-on-a-server-driven-table",
  ...
  tags: ["tables", "data", "server", "server-table", "agents"],
}
