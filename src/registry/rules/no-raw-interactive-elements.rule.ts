  ...
  replacements: [
    ...
    },
    {
      ...
      use: [
        ...
        { name: "DataTable", from: "@/components/data-table", slug: "data-table", when: "the rows you have are all of them — the browser holds the dataset and sorts, filters, pages and groups it" },
        { name: "ServerTable", from: "@/components/server-table", slug: "server-table", when: "the rows are a page of a larger answer — sorting, filtering and paging are queries the server runs" },
        { name: "TableShell", from: "@/components/table-shell", slug: "table-shell", when: "neither mode fits and you are bringing a TanStack instance of your own" },
      ],
      note: "Table is not a drop-in for <table>: react-aria throws at runtime unless exactly one Column carries isRowHeader, every Row has an id, and each row has one cell per column — so a hand-rolled table migrated without row ids crashes rather than degrades. DataTable and ServerTable do all three for you (getRowId is required, the row header is the first column, the cells come from the column list); bare Table does not. Between those two the question is where the rows come from, not how many there are — DataTable holds the answer and sorts it, ServerTable reports intent and asks again. They are two assemblies of the same parts: TableShell (@/components/table-shell) draws rows, and the toolbar, filter panels, pager and bulk bar come from @/components/table-controls — both usable directly when neither ready-made mode is the right shape. There is also no footer or caption wrapper on bare Table: a <tfoot> has no ui-lib equivalent, and because the primitive ban is derived from what the library imports, TableFooter is not banned — importing it is the honest migration, and the one place this rule sends you to react-aria rather than to a component.",
    },
  ],
  examples: [
    {
