      ...
      <Pagination>
        <PaginationList>
          ...
          {pageItems(page, range.pageCount).map((item, index) =>
            item === "gap" ? (
              // biome-ignore lint/suspicious/noArrayIndexKey: a gap has no identity of its own — its position in the window is all it is.
...
export const paginationExamples: ComponentExample[] = [
  {
    ...
    description:
      "The canonical pager: the range summary above the page numbers, centred. PaginationStack is the column; pageItems() from @/lib/data-table picks the window and where the gaps fall. Every target is a callback here, so the page is a query parameter rather than an address.",
